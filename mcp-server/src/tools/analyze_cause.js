...
exports.analyzeCauseTool = void 0;
...
async function getFromCache(cacheKey) {
    try {
      ...
      LIMIT 1
    `;
        const result = await database_js_1.pool.query(query, [cacheKey]);
        if (result.rows.length > 0) {
            // アクセスカウントを更新
            await database_js_1.pool.query('UPDATE analysis_cache SET access_count = access_count + 1, last_accessed = CURRENT_TIMESTAMP WHERE query_hash = $1', [cacheKey]);
            return {
...
// キャッシュへの保存
async function saveToCache(cacheKey, params, result, ttlHours = 24) {
    try {
        const query = `
    ...
    `;
        ...
        await database_js_1.pool.query(query, [
            cacheKey,
            'cause_analysis',
...
// メッセージ取得（セキュアな実装）
async function getMessages(timeframeDays, period) {
...
// 変化検出
function detectChanges(recentTopics, baselineTopics) {
    const changes = [];
...
function rankCauses(changes) {
    ...
    for (const change of changes.slice(0, 5)) {
        ...
        causes.push({
            ...
            timeline: change.details.examples.map(() => new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]),
...
function generateRecommendations(causes) {
    ...
    if (recommendationMap[category]) {
        ...
        recommendations.push('継続的なデータ記録により、パターンの把握が可能になります', '小さな改善から始めて、徐々に大きな変化を目指しましょう', '必要に応じて専門家のアドバイスを求めることも重要です');
    }
    return recommendations.slice(0, 3);
}
// ログ記録
async function logAnalysis(params, result, error) {
    try {
        const query = `
...
exports.analyzeCauseTool = {
    ...
    handler: async (params) => {
        ...
        try {
            ...
            // 信頼度計算
            const confidenceScore = Math.min(0.95, (recentMessages.length / 100) * 0.3 +
...
//# sourceMappingURL=analyze_cause.js.map