...
export const analyzeFatiguePatternsTool: Tool = {
  ...
  handler: async (params: FatigueAnalysisParams) => {
    ...
    try {
      ...
      const uniqueFatigueQuery = await pool.query(
        `SELECT COUNT(DISTINCT id)::INTEGER as unique_fatigue_messages
         FROM conversation_messages
         WHERE created_at >= NOW() - INTERVAL '${timeframe} days'
      ...
      return {
        summary: `【疲労分析】疲労度スコア: ${fatigueScore}/100（${fatigueType}）。過去${timeframe}日間で疲労関連の言及が${uniqueFatigueMessages}回検出されました。${
          fatigueScore > 70 ? '慢性的な疲労状態です。' : 
